(function(root, factory) {
	...
	function interpolateValues(values, year) {
		...
		var arrayIndex = this.valueBisect.left(values, year, 0, values.length - 1)
		,	currentYear = values[arrayIndex] // Gets value from array
		;

		// If no value for current year, then assume 0
		if ( ! currentYear) return 0;

		// Smooths interpolation by returning fraction values for fractional years
		// If value is first year in values, cannot calculate previous year's value, so cannot be interpolated
		if (arrayIndex > 0) {
			var prevYear = values[arrayIndex - 1] // Gets previous year's array
			,	intTime = (year - currentYear[0]) / (prevYear[0] - currentYear[0]) // Calculates time difference between previous year and current year - interpolated time
			;

			// Current year value * difference in time between next year and interpolated year
			// Plus
			// Next year value * difference in time between current year and interpolated year
			return currentYear[1] * (1 - intTime) + prevYear[1] * intTime;
		}
